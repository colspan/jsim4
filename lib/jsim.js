...
JS_IM.prototype = {
  ...
  initialize : function(formObj,methodObj){
    var _this = this;
    this.commonCommands._this = this; // 親を教えてやる
    this.imeBox = formObj;
    ...
    JS_IM_Common.addEvent(this.imeBox, "blur", acceptString, false);
    JS_IM_Common.addEvent(this.imeBox, "mousedown", acceptString, false);
    ...
    */
    if( JS_IM_Common.Browser.WebKit_iPhone ){
        JS_IM_Common.addEvent(this.imeBox, "keypress", keyProcess, false);
    }
    else if( JS_IM_Common.Browser.IE || JS_IM_Common.Browser.WebKit ){
        JS_IM_Common.addEvent(this.imeBox, "keydown", keyProcess, false);
    }
...
//        this.imeBox.onkeypress = keyProcess;
    ...
    JS_IM_Common.addEvent(this.imeBox, "keyup", ignoreKeyUp, false);
...
//    }
  ...
  execute : function( e ){
    ...
    var cmdObj    = this.JS_IM_Method.commandMapper( eventObj );
    var returnObj = this.JS_IM_Method.process( cmdObj );
    return returnObj;
  },
  ...
  statementGenerator : function( eventObj ){
    return this.JS_IM_Method.statementGenerator( eventObj );
  },
  commonCommands : {
    _this : null, // 起動時にJS_IM_Objを代入してもらう
    accept : function(){
      if( ! _this.isEnabled ) return;
      if( _this.methodObj.inlineBuffer != "" ){
        if( _this.methodObj.params.inlineInsertion ) for( i=0; i<_this.methodObj.inlineBuffer.length; i++ ) Caret.backSpace( _this.imeBox );
        var outputStr = _this.methodObj.accept();
        Caret.nowCaretPosPutWord( _this.imeBox, outputStr );
      }
    },
    toggle : function(){
      if( _this.isEnabled ) _this.disable();
      else _this.enable();
    },
    enable : function(){
      _this.isEnabled = true;
      _this.combiningStr = "";
  //    _this.originalBackgroundColor = this.imeBox.style.background;
  //   _this.imeBox.style.background = "#FFF0F0";
      _this.GUI.stateDisplay.show();
      _this.imeBox.focus();
    },
    disable : function(){
      _this.isEnabled = false;
      _this.combiningStr = "";
  //    _this.imeBox.style.background = this.originalBackgroundColor;
      _this.GUI.stateDisplay.hide();
      _this.imeBox.focus();
    },
    clear : function(){
      _this.combiningStr = ""
  ...
  process_ : function ( cmdObj ){
    var returnObj;
    if( cmdObj.func && typeof(cmdObj.func) == 'function' ){
      returnObj = cmdObj.func( cmdObj );
    }
    else{
      returnObj = null;
    }
