  ...
  process : function ( stmtObj ){ // statement実行
    ...
    if( stmtObj.commands && typeof(stmtObj.commands) == 'object' && stmtObj.commands.length > 0 ){
      ...
      for(i=0;i<stmtObj.commands.length;i++){
        ...
        this.extParams = returnObj.extParams;
        ...
        if( this.methodObj.params.inlineInsertion ){ // form update
          ...
          this.GUI.buffer.update( this.inlineBuffer );
          var elemSelectionState = Caret.getCaretXY( this.imeBox );
          this.GUI.buffer.setPosition( elemSelectionState[0], elemSelectionState[1] );
        }
        
        if( returnObj.outputStr != null && returnObj.outputStr.length > 0 ){
...
JSIM_Method.prototype = {
  ...
  initialize : function(functionObj){
    var valueList = ["init","methodName","version","language","author","commands","extension","params","statementGenerator","JSIM_Obj","localVars","utils"];
    if( functionObj ){
