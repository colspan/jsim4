...
var keyCode = {
  ...
  getKeyStatus : function( e ){
    ...
    if( JSIM_Common.Browser.IE ) e = event; // IEの場合
    var iPhoneShiftKey = false;

    // 入力文字取得
    if( JSIM_Common.Browser.WebKit_iPhone ){
        inputCode = e.keyCode;
        if( this.isAlphabetLower( inputCode ) || this.isAlphabetUpper( inputCode ) ){
            inputChar = String.fromCharCode(inputCode);
            iPhoneShiftKey = this.isAlphabetUpper( inputCode );
        }
    }
    else if( JSIM_Common.Browser.IE ){ // onkeydownからやってくる
      inputCode = e.keyCode;//( JSIM_Common.Browser.IE ? e.keyCode : e.charCode ); // IEだとkeyCode
    ...
    else if( JSIM_Common.Browser.Gecko ){// Firefox onkeypressからやってくる
    ...
    else if( JSIM_Common.Browser.WebKit ){ // Webkit
      inputCode = e.keyCode;
      if( this.isAlphabetUpper( inputCode ) ){
        inputChar = String.fromCharCode(inputCode);
    ...
    else if( JSIM_Common.Browser.Opera ){
        alert( "未実装です" );
    }
    else{
        return null;
    ...
    var keyStatus = {
      ...
      altKey : e.altKey,
      metaKey : e.metaKey,
      shiftKey : e.shiftKey || iPhoneShiftKey,
      keyCode : e.keyCode,
      charCode : e.charCode,
      inputChar : inputChar,
      inputCode : inputCode
