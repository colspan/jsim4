...
//		element = JSIM_Common.$(element);
		element.focus();
		if(JSIM_Common.Browser.IE) {
			...
			return start_length;
		}else if( JSIM_Common.Browser.Opera || JSIM_Common.Browser.Gecko  || JSIM_Common.Browser.WebKit){
...
//		element = JSIM_Common.$(element);
		if(JSIM_Common.Browser.IE){
			return function(element){
				var xy0 =JSIM_Common.cumulativeOffset(element);
				element.focus();
...
//				element = document.getElementById(element);
				...
				//////ブラウザ別処理///////
				var xy0 =JSIM_Common.cumulativeOffset(element);
				if(JSIM_Common.Browser.Gecko  || JSIM_Common.Browser.WebKit ||(parseFloat(opera.version()) >= 9.5)){
...
//		element = JSIM_Common.$(element);
		...
		var setElmStyle = function (element){
			for(var i=0;copyProps.length>i;i++){
				elmClone.style[ JSIM_Common.camelize( copyProps[i] )] = JSIM_Common.getStyle(element, copyProps[i]);
			}
			...
			}else if(JSIM_Common.Browser.WebKit){
